...
- Open GitHub Copilot Chat in the **Ask** mode and click the **+** button to clear the prompt history.
...
- From the Copilot Chat window, choose one of these options:
  ...
  1. Hover over the Copilot's response and select `Insert into Terminal` icon.
...
### Step 2: Checking code coverage of the existing unit tests

- In this lab we will check code coverage of the existing unit tests, then we will add missing unit tests, and check if adding missing unit tests increased code coverage.

- For this purpose change your copilot mode to **Ask**, if you don't have this mode set so far.

  ![Ask mode](../../img/lab-2-2-changing-mode-to-ask.png)

- Open GitHub Copilot Chat and click the + button to clear the prompt history.

- Ask the following question, but ensure that you added to the context both files: `BikeShopController.cs` and `BikeShopControllerTest.cs` using `#`

  ```text
   @workspace #BikeShopController.cs #BikeShopControllerTest.cs could you please tell me what percent of the BikeShopController code cover unit tests in BikeShopControllerTest?
  ```

- Copilot will analyze code of both files and give you the summary of missing unit tests, and code coverage at the moment

  ![Code coverage](../../img/lab-2-2-checking-code-coverage-before-completion.png)

### Step 3: Completing Unit Tests
...
- Let's do this by using `/tests` command in Copilot inline chat. Point your cursor next to `BikeShopControllerTest` class name, press `CTRL + I`, write `/tests`, specify the prompt by adding some instructions to keep test simple and not to generate too many tests

```text
Create the unit tests for #file:BikeController.cs make sure the tests will be correctly implemented without causing additional issues; we should be resetting test data after each test case in constructor to avoid issues between tests
```

  ![Controller Docs](../../img/lab-2-3-tests-inline-chat.png)

- This command will generate multiple new unit tests at once
...
- Now let's check again code coverage of the unit tests.

- Ask the following question using Copilot Ask, but ensure that you added to the context both files: BikeShopController.cs and BikeShopControllerTest.cs using #
...
- Copilot will analyze the code and will give you the approximate code coverage of the existing unit tests with the list of tested or not tested methods. After unit test completion, code percentage should be significantly increased.

  ![Code coverage](../../img/lab-2-3-checking-code-coverage-after-completion.png)

- As you can see on the above screen, after completion of the unit tests, code coverage has been increased to 65%.

### Step 4: Developing Robust Tests
...
### Step 5 - Creating the new Controller

- Open the `BikeShopAPI` project in Visual Studio Code.

- Open the `Entities/Bike.cs` file.

- Change the Copilot mode to **Agent** by clicking on the Copilot icon in the sidebar and selecting **Agent** from the dropdown menu.

  ![Controller Docs](../../img/lab-2-5-changing-to-agent.png)

- Open GitHub Copilot Chat and click the **+** button to clear the prompt history.

- Ask the following question using Copilot Agent:

  ```text
  Create an ApiController with all the CRUD operations using the Bike class, then add test data for the first three bikes directly in this controller.
  ```

- Copilot Agent will give a suggestion to create a `BikeController` class based on the `Bike` class.
...
- Agent Mode should propose name of the new file, and its correct location in the project structure. All you need to do is click `Keep` to save the changes.

  ![Controller Docs](../../img/lab-2-5-keeping-new-file.png)

- On the screen above you can see indicators suggesting that this is the new file and the place where you can keep or discard this file.

- Ensure that the file is placed in the correct location, in `Controllers` folder.
...
- Ask the following question using Copilot Agent:
  ...
  ```text
   Create the unit tests for BikeController make sure the tests will be correctly implemented without causing additional issues; we should be resetting test data after each test case in constructor to avoid issues between tests
  ```

- Copilot Agent will give a suggestion to create a `BikeControllerTests` class based on the `BikeController` class with all the needed unit tests.
...
- Ensure that the file is placed in the correct location, in `BikeShopAPI.Test/Controllers` folder
